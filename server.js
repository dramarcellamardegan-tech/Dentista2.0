...
const LINK_AGENDAMENTO = (process.env.LINK_AGENDAMENTO || `http://localhost:${PORT}`).replace(/['"]/g, '');
const DURACAO_CONSULTA_MIN = Number(process.env.DURACAO_CONSULTA_MIN || 30);
const HORARIOS_ATENDIMENTO_INICIAL = (process.env.HORARIOS_ATENDIMENTO || '17:30,18:00,18:30,19:00,19:30,20:00,20:30').split(',');
...
if (process.env.GOOGLE_APPLICATION_CREDENTIALS) {
  try {
    ...
    console.log('✅ Loaded GOOGLE_APPLICATION_CREDENTIALS from file');
  } catch (e) {
    console.warn('⚠️ Could not read GOOGLE_APPLICATION_CREDENTIALS file:', e.message);
  }
}
if (!googleClientEmail || !googlePrivateKey) {
  console.warn('⚠️ GOOGLE_CLIENT_EMAIL or GOOGLE_PRIVATE_KEY not fully provided; Sheets/Calendar may fail.');
}
...
waClient.on('qr', qr => { qrcode.generate(qr, { small: true }); console.log('🔎 Escaneie o QR com WhatsApp Web (veja o QR no terminal)'); });
waClient.on('ready', () => { clientReady = true; console.log('🟢 WhatsApp client ready'); });
waClient.on('authenticated', () => console.log('✅ WhatsApp authenticated'));
waClient.on('auth_failure', e => console.error('❌ WhatsApp auth_failure', e));
waClient.on('disconnected', reason => { console.log('🔴 WhatsApp disconnected:', reason); clientReady = false; setTimeout(() => waClient.initialize(), 3000); });
waClient.initialize();
...
async function enviarMensagemWhatsApp(numero, mensagem) {
  try {
    if (!clientReady) { console.warn('⚠️ WhatsApp not ready; skipping message to', numero); return false; }
...
async function appendRow(values) {
  ...
  return await sheets.spreadsheets.values.append({ spreadsheetId: SPREADSHEET_ID, range: `${SHEET_NAME}!A:K`, valueInputOption: 'USER_ENTERED', insertDataOption: 'INSERT_ROWS', requestBody: { values: [values] }, });
}
...
async function buscarAgendamentoPendente(telefone) {
  try {
    const clientAuth = await auth.getClient();
    const sheets = google.sheets({ version: 'v4', auth: clientAuth });
    const response = await sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: `${SHEET_NAME}!A:K`, });
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (status === 'pendente' && telefonePlanilha === telefoneLimpo) {
        return { nome: row[1], telefone: telefonePlanilha, data: row[4], horario: row[5], calendarId: row[7], linha: i + 1, email: row[3] };
      }
    }
    return null;
  ...
  try {
    ...
    for (let i = rows.length - 1; i >= 1; i--) {
      ...
      if ((status === 'pendente' || status === 'confirmado') && telefonePlanilha === telefoneLimpo) {
        return { nome: row[1], telefone: telefonePlanilha, data: row[4], horario: row[5], calendarId: row[7], linha: i + 1, email: row[3], statusAtual: row[6] };
      }
    }
    return null;
    ...
    if (agendamentoPendente) {
      ...
      if (isAff) {
        try {
          ...
          if (eventId) { await updateCell(`${SHEET_NAME}!H${linha}`, [[eventId]]); }
    ...
    if (currentState === 'AWAITING_CANCEL_CONFIRMATION') {
      ...
      if (agendamentoAtivo && isAff) {
        ...
        try {
          ...
          if (calendarId) { await deleteCalendarEvent(calendarId); await updateCell(`${SHEET_NAME}!H${linha}`, [['']]); }
...
app.post('/api/agendar', async (req, res) => {
  try {
    const { nome, telefone, email, data_agendamento, horario } = req.body;
    if (!nome || !telefone || !email || !data_agendamento || !horario) return res.status(400).json({ error: 'Todos os campos são obrigatórios.' });
    const id = uuidv4(); const criado_em = new Date().toISOString();
    const row = [id, nome, telefone, email, data_agendamento, horario, 'Pendente', '', '', '', criado_em];
...
app.post('/api/cancelar', async (req, res) => {
  try {
    ...
    for (let i = 1; i < rows.length; i++) { if (rows[i][0] === id) { linha = i + 1; agendamentoData = { id: rows[i][0], nome: rows[i][1], telefone: rows[i][2], data: rows[i][4], horario: rows[i][5], calendarId: rows[i][7], statusAtual: rows[i][6] }; break; } }
    ...
    await updateCell(`${SHEET_NAME}!G${linha}`, [['Cancelado']]);
    if (agendamentoData.calendarId) { await deleteCalendarEvent(agendamentoData.calendarId); await updateCell(`${SHEET_NAME}!H${linha}`, [['']]); }
...
async function runRemindersJob() {
  try {
    ...
    const now = new Date();
    for (let i = 1; i < rows.length; i++) {
      const row = rows[i]; const linha = i + 1; const id = row[0] || ''; const nome = row[1] || ''; const telefone = row[2] || ''; const email = row[3] || ''; const data_agendamento = row[4] || ''; const horario = row[5] || ''; const status = String(row[6] || '').toLowerCase(); const calendarId = row[7] || ''; const notificado_cliente = String(row[8] || '').toLowerCase(); const notificado_dentista = String(row[9] || '').toLowerCase();
      ...
      if (Math.abs(diffMinutes - target24) <= TOLERANCE_MINUTES && notificado_cliente !== '1') {
        const msgCliente24 = `🔔 Lembrete: Olá ${nome}, sua avaliação é amanhã às ${horario}. Caso precise alterar ou cancelar, responda por aqui.`;
        await enviarMensagemWhatsApp(telefone, msgCliente24);
        if (DENTIST_PHONE) { const msgDentista24 = `🔔 Lembrete 24h: Paciente ${nome} - ${data_agendamento} ${horario}`; await enviarMensagemWhatsApp(DENTIST_PHONE, msgDentista24); }
        try { await updateCell(`${SHEET_NAME}!I${linha}`, [['1']]); await updateCell(`${SHEET_NAME}!J${linha}`, [['1']]); } catch (e) { console.warn('⚠️ Falha ao marcar notificado 24h:', e && e.message ? e.message : e); }
      ...
      if (Math.abs(diffMinutes - target2) <= TOLERANCE_MINUTES && notificado_cliente !== '2') {
        const msgCliente2 = `⏰ Lembrete: Olá ${nome}, sua avaliação é HOJE às ${horario}. Estaremos te aguardando!`;
        await enviarMensagemWhatsApp(telefone, msgCliente2);
        if (DENTIST_PHONE) { const msgDentista2 = `⏰ Lembrete 2h: Paciente ${nome} - ${data_agendamento} ${horario}`; await enviarMensagemWhatsApp(DENTIST_PHONE, msgDentista2); }
        try { await updateCell(`${SHEET_NAME}!I${linha}`, [['2']]); await updateCell(`${SHEET_NAME}!J${linha}`, [['2']]); } catch (e) { console.warn('⚠️ Falha ao marcar notificado 2h:', e && e.message ? e.message : e); }
...
setInterval(() => { runRemindersJob().catch(err => console.warn('⚠️ runRemindersJob failed:', err)); }, REMINDER_INTERVAL_MINUTES * 60 * 1000);
runRemindersJob().catch(err => console.warn('⚠️ runRemindersJob failed at start:', err));
